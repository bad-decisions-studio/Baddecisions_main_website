...
- Computed at build time: `{{footer_year}}` (current calendar year), `{{job_posted_date}}` (today, ISO yyyy-mm-dd, used in `JobPosting` JSON-LD), `{{job_valid_through}}` (today + 90 days)
- Env-driven: `{{gsc_verification_meta}}` — emits `<meta name="google-site-verification">` if `GSC_VERIFICATION` env var is set, otherwise nothing
...
- JSON-LD on every public page:
  ...
  - `PodcastSeries` + `BreadcrumbList` (podcast)
  - `Course` with `offers` + `hasCourseInstance` + `BreadcrumbList` (education)
  - `BreadcrumbList` (work-with-us hub)
  - `Service` + `OfferCatalog` + `BreadcrumbList` (services)
  - `BreadcrumbList` (media-partnerships)
  - `BreadcrumbList` + per-role `JobPosting` (open-roles). Job dates are stamped at build time; `validThrough` is 90 days out, so re-deploys keep listings fresh in Google Jobs without manual edits.
...
| `CRON_SECRET` | Required for cron refresh | Protects `/api/cron/podcast-refresh`. Vercel Cron sends it as a bearer token (compared timing-safely). |
| `VERCEL_DEPLOY_HOOK_URL` | Required for cron refresh | Deploy hook URL called by the daily podcast refresh cron. |
| `GSC_VERIFICATION` | Optional | If set, build injects `<meta name="google-site-verification">` into every page. Get the token from Google Search Console → "HTML tag" verification method. |
